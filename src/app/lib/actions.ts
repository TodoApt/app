...
import { brandCacheTag } from "@/lib/edge-config";
...
export async function updateTeamAndBranding(
  id: string,
  prevState: FormState,
  formData: FormData,
): Promise<FormState> {
  const result = UpdateTeamSchema.safeParse({
    ...
    secondaryLight: formData.get("secondaryLight"),
    primaryDark: formData.get("primaryDark"),
    secondaryDark: formData.get("secondaryDark"),
  ...
  const { name, primaryLight, secondaryLight, primaryDark, secondaryDark, logo } =
    result.data;
  ...
  revalidateTag(brandCacheTag(process.env.BRAND_HOST || host), "max");
